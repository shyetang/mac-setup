...
- ✅ **一键初始化** - 自动安装和配置整个开发环境
- ✅ **智能配置检测** - 自动识别现有 Oh My Zsh 配置，智能合并插件列表
- ✅ **完整备份恢复** - 自动备份原始配置，支持完全回滚到运行前状态
...
**✨ 新功能：完全恢复原始配置**
- 如果运行 setup-macos.sh 时检测到现有配置，会自动备份到 `~/.mac-setup-backup/`
- rollback.sh env 会自动恢复原始的 plugins 和 theme 设置
- **真正做到完全回滚到运行前状态！**

**适用场景：** 卸载脚本配置，完全恢复初始环境
...
**新功能：** 脚本现在能够智能检测现有的 Oh My Zsh 配置，避免重复初始化冲突。

### 工作机制

**检测现有配置：**
```
▶ 检测到现有 Oh My Zsh 配置，执行智能合并
  现有插件: git docker
  现有主题: robbyrussell
▶ 备份原始配置到 ~/.mac-setup-backup
  合并后插件: colored-man-pages docker extract fzf git sudo
  
  💡 脚本推荐使用 starship（现代化命令行提示符）
  是否改用 starship？[y/N]: _
```

### 核心特性

1. **自动检测** - 识别 .zshrc 中的现有 Oh My Zsh 配置（排除 AUTO 块）
2. **智能合并** - 保留用户插件 + 添加推荐插件，自动去重
3. **尊重选择** - 询问是否使用 starship，保留用户主题偏好
4. **自动备份** - 修改前自动备份原始 plugins 和 theme 到 `~/.mac-setup-backup/`
5. **完全恢复** - `rollback.sh env` 可恢复到运行前的完整状态

### 备份文件

```bash
~/.mac-setup-backup/
├── original-plugins.20251214093048    # 带时间戳的备份
├── original-plugins.latest            # 符号链接，指向最新备份
├── original-theme.20251214093048
└── original-theme.latest
```

### 性能优化

- **16-32倍提升** - 不再调用 `brew info --cask` 检查每个包
- **完全离线** - 基于文件标记解析，无需网络连接
- **BSD awk 兼容** - 完美支持 macOS 和 Linux
...
| 文件 | 用途 | 说明 |
|------|------|------|
| `setup-macos.sh` | 主脚本 | 自动化初始化，445 行，含智能配置检测和备份功能 |
| `rollback.sh` | 回滚脚本 | 三级回滚机制，144 行，支持完全恢复原始配置 |
| **`brew-packages.txt`** | **配置文件** | **列出需要安装的软件** |
| `README.md` | 文档 | 详细使用指南（本文件） |
| `test-backup-restore.sh` | 测试脚本 | 验证备份和恢复功能是否正常工作 |
